...
namespace ContainerManagement.Web.Controllers
{
    public class RouteMastersController : Controller
    {
        ...
        public async Task<IActionResult> Import(IFormFile file, CancellationToken ct)
        {
            ...
            if (maxNonEmptyCol > 2)
            {
                TempData["Error"] = $"The file has {maxNonEmptyCol + 1} columns with data. Route import accepts exactly 3 columns: Route Name (A), Origin Code (B), Destination Code (C). Please remove extra columns and try again.";
                return RedirectToAction(nameof(Import));
            }
